...
from app.utils.document_loader import get_loader, clean_text, process_documents, enhance_document_metadata
...
async def embed_local_file(
    ...
    try:
        ...
        data = enhance_document_metadata(data, document.filename, file_ext)
...
async def embed_file(
    ...
    try:
        ...
        data = await run_in_executor(None, loader.load)
        # Enhance documents with better metadata
        data = enhance_document_metadata(data, file.filename, file_ext)
        result = await store_data_in_vector_db(
            data=data, file_id=file_id, user_id=user_id, project_id=project_id, clean_content=file_ext == "pdf"
        )
...
async def embed_file_upload(
    ...
    try:
        ...
        data = await run_in_executor(None, loader.load)
        # Enhance documents with better metadata
        data = enhance_document_metadata(data, uploaded_file.filename, file_ext)
        result = await store_data_in_vector_db(
...
async def get_project_stats(project_id: str):
    ...
    try:
        if isinstance(vector_store, AsyncPgVector):
            stats = await vector_store.get_project_stats(project_id)
        else:
        ...
        logger.error(
            "Error getting project stats | Project ID: %s | Error: %s | Traceback: %s",
            project_id,
            str(e),
            traceback.format_exc(),
        )
