...
class AtlasMongoVector(MongoDBAtlasVectorSearch):
    ...
    def get_project_stats(self, project_id: str) -> Dict[str, Any]:
        ...
        # Query documents filtered by project_id in metadata
        pipeline = [
        ...
        file_types_result = list(self._collection.aggregate(file_type_pipeline))
        file_types = {item["_id"] or "unknown": item["count"] for item in file_types_result}
        
        return {
            "project_id": project_id,
            "total_documents": len(data.get("unique_files", [])),
            "total_chunks": data.get("total_chunks", 0),
            "file_types": file_types,
            "storage_size": data.get("total_size", 0),
            "last_indexed": data.get("last_indexed").isoformat() if data.get("last_indexed") else None
        }